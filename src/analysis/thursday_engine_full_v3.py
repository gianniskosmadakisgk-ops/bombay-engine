...
#       total_lambda / lambda_gap / abs_lambda_gap
#       suitability_* scores (0..1): home/away/draw/over/under
#       suitability_flags (booleans) inside flags
#
#  Notes:
...
#   - Selection-only hints DO NOT change model probabilities; stored for Friday.
# ============================================================
...
def _suitability_scores(lam_h, lam_a, m_ph, m_pd, m_pa, m_po, m_pu, match_debug):
    ltot = (lam_h or 0.0) + (lam_a or 0.0)
    gap = (lam_h or 0.0) - (lam_a or 0.0)
    abs_gap = abs(gap)

    om_score = safe_float((match_debug or {}).get("score"), 0.0) if (match_debug or {}).get("matched") else 0.0
    om_n = _norm01(om_score, 0.60, 0.90)

    # Over: higher total + higher over prob, mildly helped by odds match
    over_s = 0.55 * _norm01(m_po, 0.42, 0.62) + 0.35 * _norm01(ltot, 2.35, 3.35) + 0.10 * om_n

    # Under: lower total + higher under prob + tighter (small abs gap) + odds match
    under_s = (
        0.45 * _norm01(m_pu, 0.55, 0.70)
        + 0.35 * (1.0 - _norm01(ltot, 2.10, 3.00))
        + 0.10 * (1.0 - _norm01(abs_gap, 0.15, 0.55))
        + 0.10 * om_n
    )

    # Draw: higher draw prob + low-ish total + small abs gap + odds match
    draw_s = (
        0.50 * _norm01(m_pd, 0.24, 0.32)
        + 0.25 * (1.0 - _norm01(ltot, 2.10, 3.10))
        + 0.15 * (1.0 - _norm01(abs_gap, 0.10, 0.45))
        + 0.10 * om_n
    )

    # Home: p_home + positive gap + odds match
    home_s = 0.60 * _norm01(m_ph, 0.28, 0.60) + 0.30 * _norm01(gap, 0.05, 0.60) + 0.10 * om_n
    # Away: p_away + negative gap magnitude + odds match
    away_s = 0.60 * _norm01(m_pa, 0.22, 0.55) + 0.30 * _norm01(-gap, 0.05, 0.60) + 0.10 * om_n

    return {
        "total_lambda": round(ltot, 3),
        "lambda_gap": round(gap, 3),
        "abs_lambda_gap": round(abs_gap, 3),
        "suitability_over": round(_clamp(over_s, 0.0, 1.0), 3),
        "suitability_under": round(_clamp(under_s, 0.0, 1.0), 3),
        "suitability_draw": round(_clamp(draw_s, 0.0, 1.0), 3),
        "suitability_home": round(_clamp(home_s, 0.0, 1.0), 3),
        "suitability_away": round(_clamp(away_s, 0.0, 1.0), 3),
        "odds_match_score": None if not (match_debug or {}).get("matched") else round(om_score, 3),
    }
...
def build_fixture_blocks():
    ...
    for fx in all_fixtures:
        ...
        # 1) MODEL probs (stabilized, NO market snap)
        ...
        suit = _suitability_scores(lam_h, lam_a, m_ph, m_pd, m_pa, m_po, m_pu, match_debug)
        ...
        flags = {
            ...
            # market suitability flags (booleans)
            "draw_suitable": bool(suit["suitability_draw"] >= 0.70),
            "under_suitable": bool(suit["suitability_under"] >= 0.72),
            "over_suitable": bool(suit["suitability_over"] >= 0.65),
            "home_suitable": bool(suit["suitability_home"] >= 0.65),
            "away_suitable": bool(suit["suitability_away"] >= 0.62),

            # keep legacy candidate hints (safe)
        ...
        fixtures_out.append(
            {
                ...
                # additive totals for downstream (Friday/Draw/Under logic)
                "total_lambda": suit["total_lambda"],
                "lambda_gap": suit["lambda_gap"],
                "abs_lambda_gap": suit["abs_lambda_gap"],

                # additive suitability scores (0..1)
                "suitability_home": suit["suitability_home"],
                "suitability_away": suit["suitability_away"],
                "suitability_draw": suit["suitability_draw"],
                "suitability_over": suit["suitability_over"],
                "suitability_under": suit["suitability_under"],
                ...
                # legacy aliases expected by current presenter (kept)
                "score_draw": evx,
                "score_over": evo,
