...
def load_history(path: str) -> Dict[str, Any]:
    ...
    try:
        h = load_json(path)
        if "week_count" not in h: h["week_count"] = 0
        if "last_window_from" not in h: h["last_window_from"] = None
        if "weeks" not in h or not isinstance(h["weeks"], dict): h["weeks"] = {}
        if "core" not in h: h["core"] = {"stake": 0.0, "profit": 0.0, "bankroll_current": None}
        if "funbet" not in h: h["funbet"] = {"stake": 0.0, "profit": 0.0, "bankroll_current": None}
        if "bankroll_current" not in h["core"]: h["core"]["bankroll_current"] = None
        if "bankroll_current" not in h["funbet"]: h["funbet"]["bankroll_current"] = None
        return h
    except Exception:
        return {
            "week_count": 0,
...
def _group_stats(items: List[Dict[str, Any]]):
    picks = len(items)
    ...
    roi_pct = round((settled_profit / settled_stake) * 100.0, 2) if settled_stake > 0 else None
    pending = sum(1 for p in items if p.get("result") == "PENDING")
    return {"picks": picks, "hits": hits, "pending": pending, "roi_pct_settled": roi_pct}
...
def main():
    ...
    # bankroll start from history if present, else from friday defaults
    ...
    # Update history only when fully settled (no PENDING)
    wk_key = window_from or week_id
    prev_week = (history.get("weeks") or {}).get(wk_key)

    if core_pending == 0 and fun_pending == 0:
        # remove previous contributions if rerun same window
        if isinstance(prev_week, dict):
            old_core_stake = safe_float((prev_week.get("core") or {}).get("stake"), 0.0) or 0.0
            old_core_profit = safe_float((prev_week.get("core") or {}).get("profit"), 0.0) or 0.0
            old_fun_stake = safe_float((prev_week.get("funbet") or {}).get("stake"), 0.0) or 0.0
            old_fun_profit = safe_float((prev_week.get("funbet") or {}).get("profit"), 0.0) or 0.0

            history["core"]["stake"] = round((safe_float(history["core"].get("stake"), 0.0) or 0.0) - old_core_stake, 2)
            history["core"]["profit"] = round((safe_float(history["core"].get("profit"), 0.0) or 0.0) - old_core_profit, 2)
            history["funbet"]["stake"] = round((safe_float(history["funbet"].get("stake"), 0.0) or 0.0) - old_fun_stake, 2)
            history["funbet"]["profit"] = round((safe_float(history["funbet"].get("profit"), 0.0) or 0.0) - old_fun_profit, 2)
    ...
    # week_count increments only when new window.from
    ...
    history["weeks"][wk_key] = {
        ...
        "fully_settled": bool(core_pending == 0 and fun_pending == 0),
        "updated_at": now_utc_iso(),
    }
    ...
    report = {
        ...
        "core": {
            ...
            "score": {
                "singles": f'{sum(1 for p in core_out_singles if p.get("result")=="WIN")}/{len(core_out_singles)}',
                "double": (f'{sum(1 for l in (core_out_double.get("legs") or []) if l.get("result")=="WIN")}/{len(core_out_double.get("legs") or [])}'
                           if core_out_double else "—")
            }
        },
        ...
        "funbet": {
            ...
            "score": {
                "singles": f'{sum(1 for p in fun_out_singles if p.get("result")=="WIN")}/{len(fun_out_singles)}',
                "system_pool": f'{sum(1 for r in fun_pool if (r.get("fixture_id") is not None))}/{len(fun_pool)}'
            }
        },
