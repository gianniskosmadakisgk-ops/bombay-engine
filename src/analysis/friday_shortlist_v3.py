...
FRIDAY_REPORT_PATH = "logs/friday_shortlist_v3.json"   # ✅ V3 ONLY (DO NOT CHANGE)

# ============================================================
#  FRIDAY SHORTLIST v3 (CORE + FUNBET ONLY)
#  - Reads Thursday JSON: logs/thursday_report_v3.json
#  - Produces: logs/friday_shortlist_v3.json
#  - NO Draw/Over/Kelly categories anymore.
#  - CORE: 4–6 singles + optional double (only legs <= 1.50)
#  - FUNBET: up to 7 picks + adaptive system (3/5, 3-4/6, 4/7, 3-4-5/7)
#  - Uses ONLY Thursday fields: probs, fairs, offered odds, value_pct_*
# ============================================================
...
def market_rows_from_fixture(fx):
    """
    Build market candidates using ONLY Thursday JSON fields.
    Markets: Home(1), Draw(X), Away(2), Over2.5, Under2.5
    Requires:
      - prob exists
      - fair exists
      - offered odds exists
      - value_pct exists (already computed in Thursday)
    """
    home = fx.get("home")
    away = fx.get("away")
    ...
    def add(market, prob_key, fair_key, odds_key, value_key):
        ...
        val = fx.get(value_key, None)  # already % or None
...
def core_stake(odds, value_pct, prob):
    """
    CORE stake scaling:
      - Higher value_pct => higher stake
      - Higher odds => lower stake
      - Higher prob => slightly higher stake
    Bounded to [CORE_STAKE_MIN, CORE_STAKE_MAX]
    """
    # value factor: 3% => ~1.0 ; 10% => ~1.4 ; 20% => ~2.0 (cap)
    vf = min(2.0, max(1.0, (value_pct / 7.0)))

    # odds factor: 1.30 => ~1.0 ; 2.20 => ~0.65
    of = max(0.55, min(1.0, 1.0 - (odds - 1.30) * 0.40))

    # probability factor: mild boost
...
def choose_core(rows):
    ...
    # optional double: only from legs <= 1.50
    ...
    if len(low) >= 2:
        ...
        for a, b in combinations(low, 2):
            # prefer value + prob, but keep simple
        ...
        if best_pair:
            ...
            # double stake: scaled down vs singles (higher combo odds => lower stake)
            dbl_stake = round(max(18.0, min(30.0, 28.0 * (1.8 / max(1.2, combo_odds)))), 1)
...
def fun_system_for_n(n, avg_value, avg_odds):
    """
    Adaptive FUN system:
      - If 7 picks strong => 3-4-5/7 (91 columns)
      - else fallback simpler.
    """
    if n >= 7:
...
def cap_core_exposure(core_singles, core_double):
    """
    If CORE open > cap, scale all stakes down proportionally.
    """
...
def main():
    log("🚀 Running Friday Shortlist v3 (CORE + FUNBET only)")
